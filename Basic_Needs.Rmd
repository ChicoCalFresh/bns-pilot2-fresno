...
```{r, fig.width=8.5}
p1 <- plot_frq(bns$q47, ylim=c(0,23), wrap.title = 40, wrap.labels = 15, 
               title="Did you apply for CalFresh EBT as an individual or with other people in your household?") +
  xlab('')
p2 <- plot_frq(bns$fed_through_EBT, ylim=c(0,11), wrap.title = 40, title="How many people in the household are regularly fed through your CalFresh EBT card?") + xlab('')
...
```{r, fig.width=7}
hist_with_box('q49_1', 6.75, 2.3, 0.2, NULL, c(0,8.5), 'Monthly Support ($)')
```
...
p1 <- plot_frq(bns$q50, ylim=c(0,30), title="Do you feel that this is enough money to provide healthy food for the month?") + xlab('')
...
```{r, fig.height=3.5}
plot_frq(bns$q54, ylim=c(0,45), wrap.labels = 15) + xlab('Food Pantry Use Frequency') + ylab("Frequency")
```
